...
router.get("/free", (req, res) => {
  ...
  var child;
  child = exec("free -b | grep Mem |  awk  '{print $3}'", function (
    error,
    stdout,
    stderr
  ) {
...
router.get("/ifstat", (req, res) => {
  ...
  var exec = require("child_process").exec;
  var child;
  child = exec("ifstat -n -b -i eth0  0.3 1 | grep [0-9]$", function (
    error,
    stdout,
    stderr
  ) {
...
router.get("/gettopo", (req, res) => {
  ...
  var exec = require("child_process").exec;
  var child;
  child = exec("curl localhost:8080/topology", function (
    error,
    stdout,
    stderr
  ) {
...
router.get("/stopvsorc", (req, res) => {
  ...
  var payload;
  console.log("erasing...");
  child1 = exec("cd $(echo $HOME/vsorcdistro)/scripts && exec 3>&- ", function (
    error,
    stdout,
    stderr
  ) {
